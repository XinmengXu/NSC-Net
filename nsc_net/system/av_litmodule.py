...
class AudioVisualLightningModule(pl.LightningModule):
    ...
    def _log_scalars(self, scalars):
        experiment = getattr(self.logger, "experiment", None)
        if experiment is None:
            return
        if hasattr(experiment, "log"):
            experiment.log(scalars)
        elif hasattr(experiment, "add_scalar"):
            step = scalars.get("epoch", self.current_epoch)
    ...
    def on_validation_epoch_end(self):
        ...
        self._log_scalars({"val_pit_sisnr": -val_loss, "epoch": self.current_epoch})

        # test
        if (self.trainer.current_epoch) % 10 == 0:
            avg_loss = torch.stack(self.test_step_outputs).mean()
            test_loss = torch.mean(self.all_gather(avg_loss))
            self._log_scalars({"test_pit_sisnr": -test_loss, "epoch": self.current_epoch})
